...
SAT AS (
		...
		FROM (
				SELECT DISTINCT 
					...
					CASE WHEN attribute_name = 'job_level' THEN value_h ELSE NULL END AS jl,
					CASE WHEN attribute_name = 'job_area' THEN value_h ELSE NULL END AS ja,
					CASE WHEN attribute_name = 'job_function' THEN value_h ELSE NULL END AS jf
				FROM RES1
				) AA
