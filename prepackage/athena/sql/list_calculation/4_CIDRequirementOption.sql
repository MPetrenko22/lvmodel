...
insert into lvmodel.m_pre_itbf_campaign_requirement_options
(
	id, 
	campaign_id, 
	cid,
...
FROM "lv-prepackage".app_lv.campaign_requirement_options q
INNER JOIN "lv-prepackage".app_lv.lists l on l.campaign_id = q.campaign_id
INNER JOIN lvmodel.m_pre_itbf_new_list_to_check j on j.list_id = l.id
...
merge into lvmodel.m_pre_itbf_campaign_demand_mask t 
using
	(
		select 
			o.cid,
		...
		from lvmodel.m_pre_itbf_campaign_requirement_options o
		where o.list_id = ?
	) s
