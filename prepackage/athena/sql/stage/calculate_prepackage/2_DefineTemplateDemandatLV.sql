...
WITH MP AS (
		...
		FROM "lv-prepackage-stage".app_lv.campaign_template_columns ctc
		INNER JOIN "lv-prepackage-stage".app_lv.campaigns ca ON ca.id  = ctc.campaign_id
		INNER JOIN "lv-prepackage-stage".app_lv.campaign_template_parameters ctp ON ctp.id = ctc.parameter_id
		INNER JOIN "lv-prepackage-stage".app_lv.campaign_templates ct ON ct.column_id = ctc.id
		INNER JOIN "lv-prepackage-stage".app_lv.campaign_template_mappings ctm ON ctm.campaign_template_id = ct.id
