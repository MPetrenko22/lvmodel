...
insert into lv_athena_stage.m_pre_itbf_final
(
	cid, 
	list_id, 
	session_id,
...
WITH NC AS (
		...
		FROM lv_athena_stage.m_pre_itbf_contact_new_collection a
		inner join lv_athena_stage.m_pre_itbf_approved_new_collection f on f.contact_id = a.contact_id and f.cid = a.cid and f.contact_approved = 1
		where  a.list_id = ?
),
CC AS (
		...
		FROM NC
		INNER JOIN lv_athena_stage.m_pre_itbf_contact_history_table h ON h.ext_entity_id = NC.ext_contact_id AND h.attribute_name = 'title' AND h.value = NC.title
		UNION ALL
		SELECT DISTINCT NC.cid, NC.list_id, NC.contact_id AS new_contact_id, h.entity_id, 'title' AS rule_type, NC.title
		FROM NC
		INNER JOIN lv_athena_stage.pre2_new_title_cm t ON t.title = NC.title
		INNER JOIN lv_athena_stage.m_pre_itbf_contact_history_table h ON h.entity_type = 'contact' AND h.attribute_name = 'title' AND h.value = NC.title
),
RES_C AS (
		...
		FROM CC
		INNER JOIN lv_athena_stage.m_pre_itbf_contact_history_table h ON CC.entity_id = h.entity_id  AND h.entity_type = 'contact'
),
RES_COM AS (
		...
		FROM NC
		INNER JOIN lv_athena_stage.m_pre_itbf_contact_history_table h ON h.ext_entity_id = NC.ext_company_id AND h.entity_type = 'company'
),
RES_T AS (
		SELECT  *
		FROM RES_C
...
RES AS (
		...
		FROM RES_T
		INNER JOIN lv_athena_stage.m_pre_itbf_template_demands D ON D.cid = RES_T.cid and D.list_id = RES_T.list_id AND D.entity_type = RES_T.entity_type AND D.field = RES_T.attribute_name AND D.value = RES_T.value_h
		LEFT JOIN lv_athena_stage.pre2_new_title_cm nt ON nt.title = RES_T.title AND RES_T.attribute_name IN ('job_level', 'job_area', 'job_function')
		UNION ALL
...
FIN as (
		...
		INNER JOIN (SELECT DISTINCT cid, list_id, country, a_country, state, a_state, job_level, job_area, job_function  FROM lv_athena_stage.m_pre_itbf_campaign_demand_mask) DM1
		...
		INNER JOIN (SELECT DISTINCT cid, list_id, industry, a_industry, employee, a_employee  FROM lv_athena_stage.m_pre_itbf_campaign_demand_mask) DM2 
		...
		INNER JOIN 
			(
				...
				inner join lv_athena_stage.m_pre_itbf_industry_dict SI on SI.industry = II.value_h and SI.sub_industry = SS.value_h
