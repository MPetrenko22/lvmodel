...
insert into lvmodel_dev.m_pre_itbf_approved_new_collection 
(
...
with LG_TIT as (
	...
	from lvmodel_dev.m_pre_itbf_contact_new_collection nc
	inner join "lv-prepackage-stage".lv_athena_stage.contacts c on c.id = nc.contact_id 
	where  nc.list_id = ?
...
APR_TIT as (
	...
	from lvmodel_dev.m_pre_itbf_contact_new_collection nc
	inner join lvmodel_dev.m_pre_itbf_approve_history h on h.email_id = nc.email_id and h.approve_type = 'title' and h.value = nc.title 
	where  nc.list_id = ?
),
LG_ADR as (
...
APR_ADR as (
	...
	from lvmodel_dev.m_pre_itbf_contact_new_collection nc
	inner join lvmodel_dev.m_pre_itbf_approve_history h on h.email_id = nc.email_id and h.approve_type = 'country' and h.value = nc.country 
	inner join lvmodel_dev.m_pre_itbf_approve_history h2 on h2.email_id = nc.email_id and h2.approve_type = 'state' and h2.value = nc.state 
	where  nc.list_id = ?
),
LG_EMP as (
...
APR_EMP as (
	...
	from lvmodel_dev.m_pre_itbf_contact_new_collection nc
	inner join lvmodel_dev.m_pre_itbf_approve_history h on h.ext_company_id = nc.ext_company_id and h.approve_type = 'employee' and h.value = nc.employee 
	where  nc.list_id = ?
)
select distinct 
	n.cid,
...
/* Skip Demands */
merge into lvmodel_dev.m_pre_itbf_approved_new_collection n
using (
			select distinct k.list_id, k.a_state, k.a_employee 
			from lvmodel_dev.m_pre_itbf_campaign_demand_mask k
			where k.list_id = ?
		) m
...
update  lvmodel_dev.m_pre_itbf_approved_new_collection 
