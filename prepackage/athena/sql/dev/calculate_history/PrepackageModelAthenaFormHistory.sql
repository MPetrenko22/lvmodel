...
delete from lv_athena_stage.m_pre_itbf_contact_history_table
;


/* Load contact history datea */
insert into lv_athena_stage.m_pre_itbf_contact_history_table
WITH CC AS (
...
JF AS (
		...
		FROM 
				...
				FROM  "lv-prepackage-stage".lv_stage_hotfix.campaign_template_mappings ctm1
...
FROM CC
INNER JOIN "lv-prepackage-stage".lv_stage_hotfix.contact_countries s on s.contact_id  = CC.contact_id
INNER JOIN "lv-prepackage-stage".lv_stage_hotfix.countries co ON co.id = s.country_id
/**/
UNION
...
FROM CC
INNER JOIN "lv-prepackage-stage".lv_stage_hotfix.companies com ON com.id = CC.company_id
...
INNER JOIN "lv-prepackage-stage".lv_stage_hotfix.companies com ON com.id = CC.company_id
INNER JOIN "lv-prepackage-stage".lv_stage_hotfix.company_industries ci ON ci.ref_id = com.industry_ref
INNER JOIN "lv-prepackage-stage".lv_stage_hotfix.industries i ON i.id = ci.sub_industry_id 
/**/
UNION
...
FROM "lv-prepackage-stage".lv_stage_hotfix.campaigns ca
INNER JOIN "lv-prepackage-stage".lv_stage_hotfix.lists l3                    ON l3."campaign_id" = ca."id" AND l3."local_type" = 3     
INNER JOIN "lv-prepackage-stage".lv_stage_hotfix.list_report_data_mapped lr  ON lr."list_id" = l3."id"
INNER JOIN "lv-prepackage-stage".lv_stage_hotfix.data_mapped_columns dmc     ON dmc."sheet_id" = lr."data_mapped_sheet_id"        
INNER JOIN "lv-prepackage-stage".lv_stage_hotfix.data_mapped_rows r          ON r."sheet_id" = lr."data_mapped_sheet_id"           
INNER JOIN "lv-prepackage-stage".lv_stage_hotfix.campaign_template_columns rtm1    ON rtm1."campaign_id" = ca."id"
INNER JOIN "lv-prepackage-stage".lv_stage_hotfix.campaign_template_parameters par  ON par."id" = rtm1."parameter_id" AND par."parameter" = dmc."title"
INNER JOIN "lv-prepackage-stage".lv_stage_hotfix.campaign_template_fields f        ON f."id" = rtm1."field_id"                     
INNER JOIN "lv-prepackage-stage".lv_stage_hotfix.contacts c3 ON c3.id = r."row_id"
INNER JOIN "lv-prepackage-stage".lv_stage_hotfix.lists l0 ON l0.id = l3.parent_id AND l0.local_type = 0
INNER JOIN "lv-prepackage-stage".lv_stage_hotfix.contacts c0 ON c0.list_id = l0.id AND c0.email_id = c3.email_id
INNER JOIN "lv-prepackage-stage".lv_stage_hotfix.companies com0 ON com0.id = c0.company_id
...
UPDATE lv_athena_stage.m_pre_itbf_contact_history_table 
SET "value" = 'USA'
...
delete from lv_athena_stage.m_pre_itbf_pv_history
;


/* Load PV history */
insert into lv_athena_stage.m_pre_itbf_pv_history
with CON as (
	select distinct ext_entity_id as ext_contact_id
	from lv_athena_stage.m_pre_itbf_contact_history_table h
...
PV as (
	...
	from CON
	inner join "lv-prepackage-stage".lv_stage_hotfix.contacts c on c.contact_id = CON.ext_contact_id
	inner join "lv-prepackage-stage".lv_stage_hotfix.phone_reasons r on r.id = c.phone_reason_id
	inner join "lv-prepackage-stage".lv_stage_hotfix.verifications v ON v.id = c.lvp
	inner join "lv-prepackage-stage".lv_stage_hotfix.companies com on com.id = c.company_id
...
delete from lv_athena_stage.m_pre_itbf_approve_history
;


/* Load Approve history */
insert into lv_athena_stage.m_pre_itbf_approve_history 
with 
APR_TIT as (
...
APR_CN as (
	...
	from "lv-prepackage-stage".lv_stage_hotfix.contacts_approves_history h
	inner join "lv-prepackage-stage".lv_stage_hotfix.countries c on cast(c.id as varchar(100)) = h.country_value
),
APR_EMP as (
